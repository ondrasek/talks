...
#   ./win.sh count  [folder]          the folder's `count` target — the strace -c twin (NtTrace64 counted, or the kernel's own counters), then fetch
...
cmd_count() { for f in $(folders "$@"); do echo "== count $f"; nmake "$f" count; done; cmd_fetch "$@"; }
